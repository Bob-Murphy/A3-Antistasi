...
{
_object = _x;
if (_unit distance _object < _distanceX) then
	{
	if ((count weaponCargo _object > 0) and !(_object getVariable ["busy",false])) then
		{
		_weaponsX = weaponCargo _object;
		for "_i" from 0 to (count _weaponsX - 1) do
			...
			if ((_basePossible in arifles) or (_basePossible in srifles) or (_basePossible in mguns) or (_potential in mlaunchers) or (_potential in rlaunchers)) then
				{
				_target = _object;
				_distanceX = _unit distance _object;
				_weaponX = _potential;
				};
			};
		};
...
while {_continue and ([_unit] call A3A_fnc_canFight) and (_unit getVariable "rearming") and (alive _truckX) and (_bigTimeout > time)} do
	{
	if (isNull _target) exitWith {_continue = false};
	_target setVariable ["busy",true];
