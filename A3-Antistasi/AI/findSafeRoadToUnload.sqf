private ["_destinationX","_originX","_radius","_dif","_roads","_road","_dist","_result","_safe","_blackList","_roadsTmp","_ok"];
...
_radius = if (!_safe) then {400} else {50};
_dif = (_destinationX select 2) - (_originX select 2);

if (_dif > 0) then
	{
	_radius = _radius + (_dif * 2);
	};

_roads = [];
while {count _roads == 0} do
	{
	_roadsTmp = (_destinationX nearRoads _radius) select {(surfaceType (position _x)!= "#GdtForest") and (surfaceType (position _x)!= "#GdtRock") and (surfaceType (position _x)!= "#GdtGrassTall")};
	{
	_road = _x;
	_ok = true;
	{
