...
if ((_leader != player) and (!_esHC)) then {_group = player};
...
_checkX = false;
//_distanceX = 500 - (([_leader,false] call A3A_fnc_fogCheck) * 450);
_distanceX = 500;
...
if (count _positionTel > 0) then
	...
	if (_positionTel distance getMarkerPos _base < 50) then
		...
		_distanceX = round (((position _leader) distance _positionX)/200);
